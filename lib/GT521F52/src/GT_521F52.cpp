/*
	Modified by Ernesto Sola-Thomas, October 16th 2023
	The library was updated to use the HardwareSerial library instead of SoftwareSerial
	This allows the library to be used with processors such as the Protenta H7
...
#ifndef __GNUC__
#pragma region -= GT521F52 Definitions =-
#endif  //__GNUC__
...
bool GT521F52::ChangeBaudRate(unsigned long baud)
{
...
bool GT521F52::CheckEnrolled(int id)
{
...
int GT521F52::EnrollStart(int id)
{
...
bool GT521F52::CaptureFinger(bool highquality)
{
...
//	2 - ID not used (no template to download
	// Not implemented due to memory restrictions on the arduino
	// may revisit this if I find a need for it
//int GT521F52::GetTemplate(int id)
//{
	// Not implemented due to memory restrictions on the arduino
	// may revisit this if I find a need for it
	//return false;
...
//	203 - Device error
	// Not implemented due to memory restrictions on the arduino
	// may revisit this if I find a need for it
//int GT521F52::SetTemplate(byte* tmplt, int id, bool duplicateCheck)
//{
	// Not implemented due to memory restrictions on the arduino
	// may revisit this if I find a need for it
	//return -1;
...
// resets the Data_Packet class, and gets ready to download
	// Not implemented due to memory restrictions on the arduino
	// may revisit this if I find a need for it
//void GT521F52::StartDataDownload()
//{
	// Not implemented due to memory restrictions on the arduino
	// may revisit this if I find a need for it
//}
...
Response_Packet* GT521F52::GetResponse()
{
	byte firstbyte = 0;
	bool done = false;
	// _serial->listen();
...
void GT521F52::SendToSerial(byte data[], int length)
{
  boolean first=true;
  Serial.print("\"");
  for(int i=0; i<length; i++)
...
void GT521F52::serialPrintHex(byte data)
{
  char tmp[16];
  sprintf(tmp, "%.2X",data);
  Serial.print(tmp);
